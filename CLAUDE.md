...
**Status**: Production for quenched gauge + Wilson fermion spectroscopy + quenched HMC + dynamical Wilson HMC. Clover fermion force for HMC is **fixed and gradient-check validated on CPU 2026-05-16** (pitfall #1; per-plane `|ratio-1|~1e-5`, stable dynamical clover HMC). Quenched clover spectroscopy works. GPU clover (Dirac + force) wired up and validated 2026-05-16 (was entirely unwired). GPU clover **spectroscopy** is host-solved (gauge downloaded D2H, then host `compute_propagator` → host CG/BiCGstab → host `apply_dirac`; no GPU solver invoked for spectroscopy) and the clover **CG** path is CPU-vs-GPU validated to ~1e-12 (all 8 channels, `fixedcfg.bin`, 2026-05-16). Use **CG** for clover spectroscopy: unpreconditioned **BiCGstab does not converge** on the clover operator at κ=0.150 (stagnates ~1e-2 at the 2000-iter cap on *both* CPU and GPU — a solver-conditioning limitation, not a GPU regression). **GPU port: fermion stack, quenched heat bath, AND HMC all validated** (built+validated on comphys 2026-05-16). The GPU Wilson-Dirac/solver/propagator/correlator path agrees with the CPU to ~1e-10 on a fixed config (RNG-free gate). GPU quenched heat bath is correct (`<P>≈0.54` at beta=5.6, matches literature; sweep-coloring bug fixed — pitfall #6). GPU HMC is now correct: dynamical `<P>≈0.53` with two-sign ΔH and ~97% acceptance, and pure-gauge HMC preserves the equilibrium of a thermalized config (the per-link/per-site cuRAND-indexing bug in `kernel_generate_momenta` is fixed — pitfall #6). CPU validated on lattices up to 8^3x16.
...
6. **Parallel-sweep coloring bug — FIXED 2026-05-16, comphys; supersedes the old "GPU gauge broken" diagnosis**: The old pitfall (`GPU <P>=0.540` vs CPU `0.427` at beta=5.6, "GPU gauge-update broken") was a **misdiagnosis with an inverted baseline**. The physically correct quenched plaquette at beta=5.6 (4⁴) is `<P>≈0.54` (matches SU(3) Wilson literature: ⟨P⟩(5.7)≈0.549). The CPU `0.427` was correct heat bath **corrupted by a broken overrelaxation operator**. Root cause: `heat_bath_sweep.cpp`, `overrelaxation_sweep.cpp`, `metropolis_sweep.cpp` (CPU) and `gpu/heat_bath_gpu.cu`, `gpu/overrelaxation_gpu.cu` (GPU) used a site even/odd parity checkerboard **while each thread updated all 4 link directions**. Link `(x,mu)` and link `(x+mu-nu,nu)` share the negative-staple plaquette and have the **same** site parity (mu and nu each flip parity once), so they were updated simultaneously. For heat bath this is a small bias; for **overrelaxation it destroys the microcanonical property** → plaquette monotonically cools (CPU beta=5.6: ovr=0→0.542, ovr=2→0.447, ovr=8→0.360). Fix: **mu-outer** loop — one `(mu, parity)` pass per launch/sweep, so concurrently-updated links never share a plaquette. Post-fix, overrelaxation is exactly microcanonical (CPU ovr=0/4/8 all `<P>=0.53587±0.00179`; GPU `0.53955±0.00132`; CPU vs GPU agree ~1.7σ, independent RNG). **GPU quenched heat bath is correct and validated.** The GPU fermion stack remains correct (RNG-free gate ~1e-10). **GPU HMC — FIXED 2026-05-16, comphys (separate bug from the sweep-coloring one).** Symptom: dynamical beta=5.6 κ=0.12 gave `<P>≈0.78` (correct ≈0.50) with systematically negative ΔH and 100% accept. **Root cause: `src/gpu/random_fields_gpu.cu` `kernel_generate_momenta` launched one thread per LINK (`idx` over `nLinks`) but indexed the cuRAND state per SITE (`randStates[idx/4]`).** The cuRAND array is allocated per site (`gpu_alloc_rand(latticeVolume,…)`), so the 4 link directions at each site were generated by 4 threads that read the *same* state and drew the *identical* random sequence → the 4 link conjugate momenta per site were identical (plus a write race on `randStates[site]`). The momentum heat-bath was rank-deficient (only ¼ of momentum DOF independent) → non-canonical HMC → wrong stationary distribution. The integrator/force were never the problem: RNG-free gauge-force gate matched CPU to 1e-10; the leapfrog is exactly reversible (forward→−Π→forward round-trips to ‖U−U₀‖²~1e-28) and symplectic (dt² ΔH scaling, no secular drift); with *deterministic identical* momenta CPU and GPU produce the same trajectory to ~1e-9. **Fix:** one thread per site, loop μ=0..3 drawing from the single advancing per-site stream — mirrors the (correct) `kernel_generate_gaussian_spinor` and heat-bath kernels. Validated: GPU dynamical `hmc_4x4x4x4.inp` `<P>≈0.53`, **two-sign ΔH, 97% accept** (was 0.78/100%); GPU pure-gauge HMC from a thermalized config (`inputs/hmc_fromthermal_b60.inp`, κ=1e-6) **stays** at `<P>≈0.58` instead of drifting to 0.78; quenched regression unchanged at `<P>≈0.54` (heat-bath path untouched). General lesson: **any GPU kernel that draws random numbers must match its thread granularity to the cuRAND-state granularity** (states are per-site; a per-link kernel must be one-thread-per-site looping the directions). RNG-free gate harness unchanged: `inputs/rngfree_fixedcfg_4x4x4x8.inp` + `inputs/gen_fixedcfg_4x4x4x8.inp`.
...
### Open Problems
1. **Clover HMC**: CPU and GPU fermion force both fixed & validated 2026-05-16 (pitfall #1). Remaining: production dynamical clover runs (autocorrelation, tuned MD step, scale setting)
...
- **The clover HMC force is FIXED and validated on BOTH CPU and GPU (2026-05-16; pitfall #1).** CPU: `gradient_check.cpp` gives `ratio = 1.0000` per plane. GPU: clover was entirely unwired (now `gpu_upload_sigma` + `clover_force_gpu.cu`), validated by RNG-free clover-Dirac CG-count match and GPU-vs-CPU clover HMC tracking. Wilson HMC also verified correct. Input key is `Clover Coefficient` (NOT `c_SW`); cold start gives F=0 (use hot/thermalized for the gradient check).
